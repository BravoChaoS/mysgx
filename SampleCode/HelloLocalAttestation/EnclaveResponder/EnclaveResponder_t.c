...
typedef struct ms_session_request_ecall_t {
	uint32_t ms_retval;
	sgx_dh_msg1_t* ms_dh_msg1;
	uint32_t* ms_session_id;
} ms_session_request_ecall_t;

typedef struct ms_exchange_report_ecall_t {
	uint32_t ms_retval;
	sgx_dh_msg2_t* ms_dh_msg2;
	sgx_dh_msg3_t* ms_dh_msg3;
	uint32_t ms_session_id;
} ms_exchange_report_ecall_t;

typedef struct ms_generate_response_ecall_t {
	uint32_t ms_retval;
	...
	uint32_t ms_session_id;
} ms_generate_response_ecall_t;

typedef struct ms_end_session_ecall_t {
	uint32_t ms_retval;
	uint32_t ms_session_id;
} ms_end_session_ecall_t;

static sgx_status_t SGX_CDECL sgx_session_request_ecall(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_session_request_ecall_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_session_request_ecall_t* ms = SGX_CAST(ms_session_request_ecall_t*, pms);
	...
	ms->ms_retval = session_request_ecall(_in_dh_msg1, _in_session_id);
	if (_in_dh_msg1) {
...
static sgx_status_t SGX_CDECL sgx_exchange_report_ecall(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_exchange_report_ecall_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_exchange_report_ecall_t* ms = SGX_CAST(ms_exchange_report_ecall_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	...
	ms->ms_retval = exchange_report_ecall(_in_dh_msg2, _in_dh_msg3, ms->ms_session_id);
	if (_in_dh_msg3) {
...
static sgx_status_t SGX_CDECL sgx_generate_response_ecall(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_generate_response_ecall_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_generate_response_ecall_t* ms = SGX_CAST(ms_generate_response_ecall_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	...
	ms->ms_retval = generate_response_ecall(_in_req_message, _tmp_req_message_size, ms->ms_max_payload_size, _in_resp_message, _tmp_resp_message_size, ms->ms_session_id);
	if (_in_resp_message) {
...
static sgx_status_t SGX_CDECL sgx_end_session_ecall(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_end_session_ecall_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_end_session_ecall_t* ms = SGX_CAST(ms_end_session_ecall_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = end_session_ecall(ms->ms_session_id);
...
SGX_EXTERNC const struct {
	...
	4,
	{
		{(void*)(uintptr_t)sgx_session_request_ecall, 0, 0},
		{(void*)(uintptr_t)sgx_exchange_report_ecall, 0, 0},
		{(void*)(uintptr_t)sgx_generate_response_ecall, 0, 0},
		{(void*)(uintptr_t)sgx_end_session_ecall, 0, 0},
	}
};
