...
sgx_status_t create_session_ecall(sgx_enclave_id_t eid, uint32_t* retval)
{
	sgx_status_t status;
	ms_create_session_ecall_t ms;
	status = sgx_ecall(eid, 0, &ocall_table_EnclaveInitiator, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}

sgx_status_t message_exchange_ecall(sgx_enclave_id_t eid, uint32_t* retval)
{
	sgx_status_t status;
	ms_message_exchange_ecall_t ms;
	status = sgx_ecall(eid, 1, &ocall_table_EnclaveInitiator, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}

sgx_status_t close_session_ecall(sgx_enclave_id_t eid, uint32_t* retval)
{
	sgx_status_t status;
	ms_close_session_ecall_t ms;
