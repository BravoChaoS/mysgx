...
typedef struct ms_create_session_ecall_t {
	uint32_t ms_retval;
} ms_create_session_ecall_t;

typedef struct ms_message_exchange_ecall_t {
	uint32_t ms_retval;
} ms_message_exchange_ecall_t;

typedef struct ms_close_session_ecall_t {
	uint32_t ms_retval;
} ms_close_session_ecall_t;
...
static sgx_status_t SGX_CDECL sgx_create_session_ecall(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_create_session_ecall_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_create_session_ecall_t* ms = SGX_CAST(ms_create_session_ecall_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = create_session_ecall();


	return status;
}

static sgx_status_t SGX_CDECL sgx_message_exchange_ecall(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_message_exchange_ecall_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_message_exchange_ecall_t* ms = SGX_CAST(ms_message_exchange_ecall_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = message_exchange_ecall();


	return status;
}

static sgx_status_t SGX_CDECL sgx_close_session_ecall(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_close_session_ecall_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_close_session_ecall_t* ms = SGX_CAST(ms_close_session_ecall_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = close_session_ecall();
...
SGX_EXTERNC const struct {
	...
	3,
	{
		{(void*)(uintptr_t)sgx_create_session_ecall, 0, 0},
		{(void*)(uintptr_t)sgx_message_exchange_ecall, 0, 0},
		{(void*)(uintptr_t)sgx_close_session_ecall, 0, 0},
	}
};
